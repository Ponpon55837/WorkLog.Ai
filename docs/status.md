...
| E. 資料生命週期 | 進行中 | E1 永久刪除專案與資料（先備份、輸入名稱確認、MCP 無刪除工具）已實作，PR 待 CI／合併；E2 `pnpm db:maintain` 尚未開始。 |

暫緩項目：A2 開機自動啟動、發行 workflow／tag／release，以及實機平台驗證。完成 E 階段後，另修正報告選擇不限日期時的搜尋錯誤與自訂區間切換造成的版面位移，並逐頁檢查表格是否在內部捲動，必要時補充 UI skill。D4 使用 20 題虛構合成檢索品質評估；私有 36 題真實資料檢索評估仍留在本機，不會放進 repository。
