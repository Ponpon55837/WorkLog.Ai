import { Check, CircleCheck, CircleDashed, CircleMinus, CircleSlash, CircleX, FileQuestion, Folder, FolderGit2, FolderX, GraduationCap, ListOrdered, LoaderCircle, Scale, Shapes, TriangleAlert } from "lucide-vue-next";
import type { KnowledgeKind, KnowledgeStatus, MetadataBackfillRequest, ProjectStatus, ReportSynthesisRequest, ReportVerificationStatus } from "@work-intelligence/core";
...
export function verificationOf(session: { verification?: { status: ReportVerificationStatus } }): ReportVerificationStatus {
  return session.verification?.status ?? "not_supplied";
}

/** Splits the API's `verification` gap into 未回報 vs 明確未執行 (README requirement 8). */
export function metadataGapsOf(item: { gaps: readonly ("changed_files" | "verification")[]; verificationStatus: ReportVerificationStatus }): MetadataGapKind[] {
