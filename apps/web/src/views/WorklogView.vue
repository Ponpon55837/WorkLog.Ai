...
import { useSessions, type DatePickerTarget } from "../composables/useSessions";
import { displayDate, formatDate, formatReadableSummary, toDateInputValue } from "../utils/format";
import { listPageSizeOptions, verificationLabel } from "../utils/labels";
...
function toggleDatePicker(target: DatePickerTarget): void {
  togglePicker(target, target === "from" ? dateFromTrigger.value : dateToTrigger.value);
}
...
<template>
  <section class="page-section">
    <div class="section-intro worklog-intro">
      ...
      <form class="worklog-tools" @submit.prevent="load(true)">
        <label class="search-box">
          <span>⌕</span>
          <input :value="searchTerm" type="search" placeholder="搜尋 title、summary 或 event" @input="searchTerm = (($event.target as HTMLInputElement).value)" />
        </label>
        <div class="filter-row">
          <label class="filter-field">
            <span>專案</span>
            <select :value="selectedProjectId" aria-label="依專案篩選" @change="selectedProjectId = (($event.target as HTMLSelectElement).value)">
            ...
            <Teleport to="body">
              <div v-if="activeDatePicker === 'from'" class="date-picker-popover" :style="datePickerStyle" role="dialog" aria-label="選擇開始日期" @click.stop>
                ...
                <div class="calendar-grid">
                  <button v-for="day in calendarDays" :key="day.value" :class="['calendar-day', { muted: !day.inCurrentMonth, today: day.isToday, selected: day.isSelected }]" type="button" :aria-label="`${day.value}`" @click="selectCalendarDate(day.value)">{{ day.label }}</button>
                </div>
    ...
    <section class="panel worklog-panel">
      ...
      <VirtualList :items="sessions" :enabled="sessionPageSize === 'all'" aria-label="工作歷程清單">
        <template #default="{ item: session }">
          <button class="worklog-row" data-testid="session-row" type="button" @click="openSession(session)">
      ...
      <div class="pagination-bar list-pagination-bar">
        ...
        <label class="pagination-page-size"><span>每頁</span><select :value="sessionPageSize" aria-label="工作歷程每頁筆數" @change="changeSessionPageSize($event)"><option v-for="option in listPageSizeOptions" :key="String(option.value)" :value="option.value">{{ option.label }}</option></select></label>
