...
const workRecordContract =
  "Work record contract: summary is one outcome-first executive sentence. workSummary must have exactly five arrays: outcomes (成果: confirmed deliveries or resolved problems with accurate completion wording), scope (範圍: important modules/APIs/UI/tests and concise verified change counts), decisions (決策: explicit choices and source-stated rationale only), verification (驗證: actual commands/results/counts/platform coverage and unverified limits), nextSteps (compatibility field displayed as 狀態／未結項: only objective unresolved items, known limitations, missing evidence, or unverified scenarios at finalization; never invent recommendations, plans, or future outlook). Put one concise confirmed fact in each item, do not repeat the same fact across sections, and use [] when no supported fact exists. Preserve distinctions between completed, implemented, investigated, fixed, verified, failed, and not verified. Keep verification.status as the machine-readable result. Git is optional separate metadata and must only contain observed branch/commit/dirty values; changedFiles is not proof of a commit. Do not infer facts from titles or changed-file counts.";
const reportSynthesisContract =
  "Report synthesis contract v3: write Traditional Chinese by default and synthesize directly from source Sessions, events, evidence, and deterministic report data; never summarize one report period to produce another. Group at the correct scale: daily Task/Feature/Issue, weekly Feature/Workstream, monthly Project/Milestone, quarterly Initiative, annual Major Contribution. Re-cluster related work across the entire period instead of repeating chronology or Session titles. Start with period/scope and an outcome-first executive summary. Use themes for period-level work groups; highlights for supported outcomes and significant scope; verification for exact evidence and partial/unverified coverage; comparison only when deterministic prior-period data exists; risks for evidenced risks; decisions for explicit choices and documented trade-offs. The legacy nextSteps field means only confirmed current status/open items/limitations and must not contain recommendations or future plans; use [] if none. Every material block must cite its supporting sourceSessionIds; if context is truncated, disclose incomplete coverage and do not claim exhaustiveness. Keep passed/failed/not_run/not_supplied distinct, never infer Git commits from changedFiles, do not estimate metrics, and write 資料不足 when the source is insufficient. Use promptVersion report-synthesis-v3.";
...
server.registerTool(
  "work_update_session_work_summary",
  {
    ...
    description:
      "Update the structured five-section workSummary of an existing finalized Session without creating a duplicate. Use mode=replace with all five sections to replace the complete structure, or mode=patch with one or more confirmed sections; patch merges with the current values and a legacy missing workSummary starts from empty arrays. This tool preserves the original Session id, idempotencyKey, changedFiles, verification, events, evidence, handoff snapshots, and Git metadata. It has its own idempotencyKey; retrying the same operation with the same payload is safe and never repeats a change. The existing Session project must be tracked; unregistered, paused, and ignored projects are skipped quietly. " +
      workRecordContract,
    inputSchema: updateSessionWorkSummaryInputSchemaBase.shape,
  },
  async (input) => {
    const parsed = parseMcpInput(updateSessionWorkSummaryInputSchema, input);
    if (!parsed.success) {
      return {
        isError: true,
        ...textResult({ error: "Invalid session workSummary payload.", details: parsed.error.flatten() }),
      };
    }
    return sessionTextResult(store.updateSessionWorkSummary(parsed.data));
  },
);
...
server.registerTool(
  "work_get_graph",
  {
    ...
    description:
      "Build a read-only graph from tracked project metadata: Projects, finalized Sessions, explicit Knowledge, attached Evidence, and changed-file paths. This tool does not read source, handoff, or Git files and does not infer semantic relationships; projectRoot/projectId scopes are policy-gated and non-tracked projects are skipped quietly. For large graphs, set pageSize (maximum 500) and use the returned nextCursor for incremental pages; do not ask the user to provide internal node offsets.",
...
server.registerTool(
  "work_list_metadata_backfill_requests",
  {
    ...
    description:
      "Implementation detail for natural-language Work Intelligence metadata follow-ups. When the user asks to補齊、更新、檢查或處理 metadata 缺口, automatically use this to find the newest pending or processing request without asking the user for a requestId. The user must not be exposed to this tool name or call order. " +
      metadataBackfillContract,
...
server.registerTool(
  "work_get_metadata_backfill_context",
  {
    ...
    description:
      "Implementation detail for completing a metadata backfill request. Obtain the bounded list of tracked Sessions with current metadata and unresolved gaps before inspecting their worktree, diff, or handoff evidence. This advances a pending request to processing and policy-gates project access. Do not infer changed files from the returned counts or text. " +
      metadataBackfillContract,
...
server.registerTool(
  "work_cancel_metadata_backfill",
  {
    ...
    description:
      "Cancel a pending or processing metadata backfill request when the user has no data to correct or no longer wants to proceed. Cancellation never changes the existing Session metadata; subsequent context/apply attempts are rejected until a new request is created. The project policy is checked first and unregistered, paused, or ignored projects are skipped quietly. This is an implementation detail; do not ask the user for the tool name or request ID. " +
      metadataBackfillContract,
...
server.registerTool(
  "work_save_report_summary",
  {
    ...
    description:
      "Implementation detail for completing a Work Intelligence report synthesis request. Save a grounded summary with period-specific themes, highlights, verification, supported comparison, risks, decisions, current-state/open-item blocks (legacy nextSteps field), generated agent/model metadata, prompt version, and sourceSessionIds. Do not modify original Sessions, Events, Evidence, Knowledge, or handoff snapshots. Retries for the same completed request are idempotent. Every material block should cite one or more sourceSessionIds; an evidence-free block must explicitly say 資料不足. " +
      reportSynthesisContract,
